...
# -I. first so that our local patched logging.h is found before the framework's
...
# Create patched logging.h using Python (works identically on macOS and Linux,
# unlike sed which has BSD vs GNU differences in escaping)
python3 -c "
import sys
...
with open('logging.h', 'w') as f:
    f.write(content)
"

# Verify the patch worked
echo "=== Verifying patched logging.h ==="
grep 'param_double\|param_int' logging.h
echo "=== End verify ==="
