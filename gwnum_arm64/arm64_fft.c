...
/* Thread-local flag: set to 1 by case 2 (N/2 real FFT squaring) before
   calling normalize, 0 by case 3/4 (N-point complex multiply).
   The normalizer uses this to decide whether to apply k-factor for k>1. */
static ARM64_THREAD_LOCAL int arm64_used_real_fft = 0;

int arm64_get_used_real_fft(void) {
	return arm64_used_real_fft;
}
